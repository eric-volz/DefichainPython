...
def _unhexlify(integer: int):
    try:
        return binascii.unhexlify("0%x" % integer)
...
def generate_entropy(strength: int = 128) -> str:
    ...
    return binascii.hexlify(os.urandom(strength // 8)).decode()
...
def is_entropy(entropy: str) -> bool:
    ...
    try:
        return len(binascii.unhexlify(entropy)) in [16, 20, 24, 28, 32]
    except:
        return False
...
def get_entropy_strength(entropy: str) -> int:
    ...
    length = len(binascii.unhexlify(entropy))
    if length == 16:
        return 128
    elif length == 20:
        return 160
...
def entropy_to_mnemonic(entropy: str, language: str = "english") -> str:
    ...
    return Mnemonic(language=language).to_mnemonic(binascii.unhexlify(entropy))
